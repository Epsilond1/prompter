...
als.table_fixed_header = {
	...
	item: function(content){
		...
		t.header = $('<div style="position: absolute; z-index: 2; top: 0; left: 0;"><div class="slider_wrap" style="overflow: hidden;background-color: #fff;">' + content.closest('div').html() + '</div><div class="shadow" style="height: 16px;"></div></div>').insertAfter(content);
...
als.table_fixed_header.item.prototype = {
	onscroll: function(){
		var t = this;
		var delta = als.table_fixed_header.window_scroll_top - t.content_top;
		if(delta > 0){
			if(!t.scrolled){
				t.scrolled = true;
			...
			if(t.fixed){
				delta = delta + $('.js-header_preview_menu').height();
				if (delta + t.header_content_height > t.content_height) {
					delta = $('.js-header_preview_menu').height() + t.content_height - t.header_content_height - delta;
				} else {
					delta = $('.js-header_preview_menu').height() + 0;
				}
				t.header.css({
					top: delta + 'px',
					left: t.content_left - als.table_fixed_header.window_scroll_left + 'px'
					//left: t.content_left - ($('.post_single .post__content').scrollLeft() ? $('.post_single .post__content').scrollLeft() : 0) + 'px'
				});
			}else{
				t.header.css('top', delta + 'px');
			}
		}else{
			t.header.css({position: 'absolute', top: 0, left: 0});
			t.scrolled = false;
		}
	},
	...
	onresize: function(){
		...
		t.content_left = t.content.offset().left;
		...
		t.header_content.width(t.content.width());
...
function calculateContent() {
		var elem;
		if ($('.js-calculate_content').length) {
			elem = $('.js-calculate_content');
		} else if($('.post__content').length) {
			elem = $('.post__content');
		} else if($('.content__inner').length) {
			elem = $('.content__inner');
		}
		var width = $(elem).width();
		//force min-width
		var otherContentWidth = $('.page').width() - width; // full page - main content col
		var thisContentWidth;
		var pageWidth = $('.page').innerWidth();
		var maxContent;
		var maxContentWidth = 0;
		if ($(elem).length) {
		$(elem.selector + ' table,' + elem.selector + ' img').not('.not_display table, .not_display img, .fotorama__img, .js-not_calculate, .lead-in img, .head-picture img, .slider_wrap table').each(function(){
			thisContentWidth = $(this).width();
			if ( thisContentWidth > width && thisContentWidth > maxContentWidth) {
				maxContentWidth = thisContentWidth;
				maxContent = $(this);
			}
		});
		//if (maxContentWidth) $('.page, .layout').css('min-width', maxContentWidth + otherContentWidth + 'px');
		if (window.matchMedia && !mediaMobile.matches) {
			if (maxContentWidth) {
				$('.page, .layout').css('min-width', pageWidth + maxContentWidth - width + 100 + 'px');
				maxContent.closest(elem).css('overflow', 'visible');
			}
		} else {
			if (maxContentWidth) {
				$('.page, .layout').css('min-width', '');
				maxContent.closest(elem).css('overflow', 'auto');
			}
		}
		}
}
...
jQuery(document).ready(function(){
	...
	$('.browser_outdate').on('click', function(){
		$.cookie('browser_outdate_delay', 1, { expires: 1, domain: 'hse.ru', path: '/'});
 ...
 * Build: http://modernizr.com/download/#-borderimage-boxshadow-rgba-generatedcontent-cssgradients-csstransforms-csstransforms3d-csstransitions-canvas-inlinesvg-touch-cssclasses-prefixed-teststyles-testprop-testallprops-prefixes-domprefixes-css_backgroundsizecover-css_displaytable-cssclassprefix:detect_
 */
		...
		return $.get(url, data)
					...
					.done(function(r){
						...
						$categories.on('click', 'a', function(event) {
							...
							$navigation.eq(index).find('h2').each(function() {
							...
							// update content
							$navigation.hide();
							var $selected_category = $navigation.find('h1:contains("' + $this.text() + '")').closest('li');
						...
						$subcategories.on('click', 'a', function(event) {
							...
							var $sub = $navigation.find('h1,h2,h3').filter(':contains("' + $this.text() + '")');
						...
						$('.popup_sitemap_content h1').each(function() {
...
$(function() {
	...
	$('.control_vision').on('click', function(){
...
$(function () {
	var fotorama = $('.fotorama[data-auto="false"]');
	...
	(function ($) {
		...
		$ (function () {
			...
			$ ('.emerge').each (function () {
				...
				$self.find ('*').addBack ().not ('.emerge-item *').each (function () {
					var $el = $ (this);

					// img elements
					if ($el.is ('img:uncached')) if ($el.attr ('src')) {
						innerImagesSrcs[$el.attr ('src')] = true
					}

					// emerge-items
					if ($el.is ('.emerge-item')) {
						log ('EMERGE ITEM FOUND WITH EVENT ' + $el.data ('emerge-item-event'))
						elementsCount ++
						$el.on ($el.data ('emerge-item-event'), element)
				...
				if (Object.keys && Object.keys (innerImagesSrcs).length > 0) if (spin = $self.data ('spin')) {
...
$('.switcher').each(function(){
	new als.Switcher(
		...
		jQuery(this).next('.views').children('.events')
	);
})
});
...
$(function(){
	...
	if (("ontouchstart" in document.documentElement)) {
		document.documentElement.className = "detect-touch";
			//Вешаем стрелочку у пункта меню
			$('.fa-sidemenu__arr').click(function(e) {
				if (!$(this).parent().hasClass('js-active')) {
					//$('.fa-sidemenu__item--parent').not(this).removeClass('js-active');
	...
	//var hasSidebar = $('body').hasClass('has-m_sidebar') ? 1 : 0;
	...
	function closeMobileMenu() {
		...
		$('.js-mobile_popup').removeClass('is-open');
		//collect();
		return false;
	}
	...
	function footerCarousel() {
		if(mediaMobile && window.matchMedia) {
			if (mediaMobile.matches && !footerSliderFlag) {
				...
				$(".fa-footer__menu_inner").addClass('owl-carousel').owlCarousel({
					items: 1,
					dots: true,
					nav: false,
					loop: true,
				...
				$(".fa-footer__menu_inner").trigger('destroy.owl.carousel');;
